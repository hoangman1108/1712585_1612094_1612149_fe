...
export default function DetailClass() {
  ...
  const handleImportStudents = (e) => {
    ...
    console.log("file formData", file, formData);
    classService.importLstStudentReal(formData).then((response) => {
      const status =
  ...
  useEffect(() => {
    classService.detailClass(classID).then((response) => {
  ...
  const callGetLstStudentsReal = (classID) => {
    classService.getLstStudentsReal(classID).then((response) => {
        ...
        <Container>
          ...
          <Row className="mt-5 mb-2">
            ...
            <Col className="d-flex justify-content-end">
              <ExportData
                rows={studentData}
                lstKeysRemove={["password", "passwordSalt", "id"]}
              ></ExportData>
              {me.role === "teacher" && (
                <Button
                  variant="success"
          ...
          <InviteClassModal
            ...
            isInviteTeacher={isInviteTeacher}
